...
def test_rules() :
    ...
    assert view.rule3.text == "3) Use Up, Down, Left, and Right arrow keys or W,A,S,D keys to move the player"
    assert view.rule4.text== "4) Have fun!"

def test_game_launch() :
    window = arcade.Window(1200, 650, "Test Window")
    view = Rule_Page(1)
    window.show_view(view)
    assert view.play_button.on_click == view.game_open(None)
